...
export default function SurgeryQuestionsPage() {
  ...
  useEffect(() => {
    import('./questionData').then(m => {
      try {
        setModules(parseQuestions(m.default, 'surgery', 'surgery-questions.md'));
  ...
  if (loading) return <InternalPage title="Surgery" subtitle="Loading..." introText="" backHref="/interns/jmc-exam-practice" backLabel="JMC Practice"><p className="text-slate-400 text-center py-8">Loading modules...</p></InternalPage>;
  if (error) return <InternalPage title="Surgery" subtitle="Error" introText="" backHref="/interns/jmc-exam-practice" backLabel="JMC Practice"><GlassCard><p className="text-red-400">{error}</p></GlassCard></InternalPage>;

  return (
    <InternalPage
      title="Surgery JMC Questions"
      subtitle="Surgery — Internship / JMC"
    ...
    >
      <div className="mb-6 flex items-center gap-2">
        ...
        <span className="text-xs text-slate-500 font-bold">Source: surgery-questions.md</span>
      </div>
      ...
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
        {modules.map((mod) => (
          <Link
            key={mod.id}
            href={`/interns/jmc-exam-practice/surgery/${mod.id}`}
