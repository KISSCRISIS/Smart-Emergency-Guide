...
export default function IMQuestionsPage() {
  ...
  useEffect(() => {
    import('./questionData').then(m => {
      try {
        const parsed = parseQuestions(m.default, 'internal-medicine', 'internal-medicine-questions.md');
        setModules(parsed);
  ...
  if (loading) return <InternalPage title="Internal Medicine" subtitle="Loading..." introText="" backHref="/interns/jmc-exam-practice" backLabel="JMC Practice"><p className="text-slate-400 text-center py-8">Loading modules...</p></InternalPage>;
  if (error) return <InternalPage title="Internal Medicine" subtitle="Error" introText="" backHref="/interns/jmc-exam-practice" backLabel="JMC Practice"><GlassCard><p className="text-red-400">{error}</p></GlassCard></InternalPage>;

  return (
    <InternalPage
      title="Internal Medicine JMC Questions"
      subtitle="Internal Medicine — Internship / JMC"
      introText={`Select a module below to begin. ${totalQuestions} questions across ${modules.length} modules.`}
      backHref="/interns/jmc-exam-practice"
      backLabel="JMC Practice"
    >
      <div className="mb-6 flex items-center gap-2">
        <FileText size={14} className="text-slate-500" />
        <span className="text-xs text-slate-500 font-bold">Source: internal-medicine-questions.md</span>
      </div>
      ...
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
        {modules.map((mod) => (
          <Link
            key={mod.id}
            href={`/interns/jmc-exam-practice/internal-medicine/${mod.id}`}
